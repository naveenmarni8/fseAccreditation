...
class UsernamePwdAuthenticationProviderTest {
	...
	{
		AuthenticationRequest authRequest=new AuthenticationRequest("hello.in@gmail.com","password");
		when(usersRepository.findByEmail("hello.in@gmail.com")).thenReturn(Collections.emptyList());
		assertThrows(BadCredentialsException.class,()->{
			usernamePwdAuthenticationProvider.authenticate(authRequest);
		});
	}
