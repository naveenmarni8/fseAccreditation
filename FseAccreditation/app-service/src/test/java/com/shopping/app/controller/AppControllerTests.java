...
class AppControllerTests {
	...
	{
		when(productService.deleteProductService(product.getId())).thenReturn("Product is deleted");
		ResultActions response=mockMvc.perform(delete("/api/v1.0/shopping/delete/{id}",product.getId())
				                      .with(csrf()));
